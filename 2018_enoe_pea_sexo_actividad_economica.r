...
enoe_pea_sex_actividad_salario <- enoe_2018[clase1 == 1 & sex %in% c(1, 2) & scian %in% c(1:21), .(sex, scian)]
...
g <- ggplot(conteo_enoe_sex_actividad, aes(x = `Actividad económica`, y = Frecuencia, fill = Sexo)) +
  ...
  labs(title = "Distribución de hombres y mujeres por actividad económica (2018)",
...
ggsave("2018_enoe_pea_sexo_actividad.pdf", plot = g, device = "pdf", width = 10, height = 8)