...
enoe_2016 <- rbindlist(list(f_1, f_2, f_3, f_4))

# Filtrar y seleccionar columnas
enoe_pea_sex_actividad_salario <- enoe_2016[clase1 == 1 & sex %in% c(1, 2) & scian %in% c(1:21), .(sex, scian)]
...
g <- ggplot(conteo_enoe_sex_actividad, aes(x = `Actividad económica`, y = Frecuencia, fill = Sexo)) +
  ...
  labs(title = "Distribución de hombres y mujeres por actividad económica (2016)",
...
ggsave("2016_enoe_pea_sexo_actividad.pdf", plot = g, device = "pdf", width = 10, height = 8)